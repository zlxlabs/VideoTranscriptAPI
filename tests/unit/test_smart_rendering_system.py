...
from src.video_transcript_api.utils.cache import (
...
def create_test_cache_directory():
    ...
    funasr_data = [
        {
            "spk": "speaker1",
            "text": "嗯，欢迎来到知行小酒馆，这是一档有知有行出品的播客节目。我是羽白。",
        },
        {
            "spk": "speaker2",
            "text": "承认自己是弱者，不是自我否定，反而能让人活得更有生命力。",
        },
        {
            "spk": "speaker1",
            "text": "这是中年危机的一种褒义的说法。终于我们把小酒馆听众最热爱的嘉宾之一少楠又请回来了。",
        },
    ...
    try:
        ...
        if mapping:
            ...
            if expected_speakers == actual_speakers:
                ...
                print(
                    f"⚠ 映射关系异常: 期望 {expected_speakers}, 实际 {actual_speakers}"
                )
        else:
            print("✗ 说话人映射推断失败")
...
def test_cache_analysis():
    ...
    try:
        ...
        print(f"存在的文件: {capabilities.files_present}")
...
def test_structured_data_rendering():
    ...
    try:
        ...
        structured_data = {
            "format_version": "v2",
            "video_metadata": {"video_title": "知行小酒馆测试", "author": "有知有行"},
            "speaker_mapping": {"speaker1": "知白", "speaker2": "少楠"},
            "dialogs": [
                {
                    "speaker": "知白",
                ...
                {"speaker": "少楠", "content": "是的，小酒馆是对不起的，对吧？"},
            ],
        }
        ...
        # 检查关键元素
        success = (
            "dialog-container" in html_output
            and "speaker-tag" in html_output
            and "知白" in html_output
            and "少楠" in html_output
            and "知行小酒馆" in html_output
        )
