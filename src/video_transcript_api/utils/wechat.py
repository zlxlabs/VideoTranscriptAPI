...
class WechatNotifier:
    ...
    def send_text(self, content):
        ...
        """
        return self.send_markdown_v2(content)
    ...
    def send_markdown_v2(self, content):
        ...
        if self.use_rate_limit:
            # 使用限流发送
            success = send_rate_limited_message(self.webhook, content, msgtype="markdown_v2")
            if success:
                logger.debug(f"markdown_v2消息已加入限流队列: {content[:30]}...")
            else:
                logger.error(f"markdown_v2消息加入限流队列失败: {content[:30]}...")
            return success
        else:
            # 直接发送（原有逻辑）
