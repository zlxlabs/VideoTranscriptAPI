...
- [ ] **T1 (P1)** — 阶段一 A1：CapsWriter `extra_json_data` 接通 + `get_cache` 读回 segments（经 `load_segments`）— `api/services/transcription.py`、`cache/cache_manager.py` — 单测+集成
...
- [ ] **T6 (P1)** — 状态/缓存/API/补层接入 — **按 §5.5 + §5.7**（Options 在 `processing_options.py`；media_lock/write-ahead；need_chapters 状态敏感；recalibrate 强制重算 GENERATED chapters）
- [ ] **T7 (P1)** — 前端章节区块 + `#dlg-{i}` + TOC DOM API（XSS）— §5.6
- [ ] **T8 (P2)** — 阶段二：has_speaker 全链路 + 独立分块参数 + 配置开关
- [ ] **T9 (P2)** — 真实样本 3–5 例质量验收 → prompt 定稿
- [ ] **T10 (P2)** — `docs/features/chapters.md` + `processing_options.md` 补充 chapters
