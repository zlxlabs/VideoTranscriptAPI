...
> **2026-07-22 更新（下载阶段解耦）**：普通转录流程不再在下载状态通知前预取
> `get_download_info()`。BBDown 模式下该调用可能直接执行完整下载，因此预取会造成用户长时间看不到
> “正在下载”，并可能在异常后再次触发一次下载。现在下载信息只在实际下载阶段获取一次；元数据仍由
> Bilibili 官方 API 提供，避免元数据和下载相互拖累。
