...
项目调用 BBDown 时会先从输入链接提取 BV 号，并始终传入形如
`https://www.bilibili.com/video/BV...` 的 canonical URL；即使输入是 `b23.tv` 短链，BBDown
也不会再次执行短链跳转。从已解析的长链中保留 `p` 参数以选择分 P；原始短链的 `p` 参数若在重定向
目标中丢失，属于已知 backlog。

`POST /api/transcribe` 会在受理时完成这一次 URL 解析，并把得到的 canonical URL、平台和视频 ID
随内存任务传给后台 worker。worker 会直接复用该结果，不会对同一个 `b23.tv` 输入再次跳转；原始 URL
仍用于任务创建通知和用户展示。

一次下载最多启动 3 个全新的 BBDown 进程，但它们共同使用 `bbdown.timeout`（默认 300 秒）这一
总时间预算，约束 BBDown 进程尝试与固定退避，不会把超时放大成三倍。成功后的 `ffprobe` 媒体验证
最多还可额外耗时 30 秒，尚未纳入该预算，已记入 backlog。每次尝试都有独立临时目录；非零退出、无
活动停滞或产物无效时，生产 Linux 会先清理整个进程组，再删除尝试目录，并在剩余预算内进行短暂固定
退避后重试。成功后立即停止重试。

监督器同时观察 BBDown 的 stdout/stderr 与临时目录中文件的大小、修改时间：下载前连续约 20 秒无
活动视为预取停滞；发现下载文件或进展后，阈值提升到约 60 秒。因此持续增长的媒体文件不会被预取
阈值误杀。每次命令会加上 `--skip-subtitle`、`--skip-cover`、`--skip-ai`，以减少本转录流程不需要的
请求。

进程以 0 退出并不单独构成成功：必须找到非空的候选媒体文件，且通过项目统一的 `ffprobe` 媒体校验。
失败信息会包含最后一次尝试数和失败阶段，并只保留有限的 BBDown 输出尾部用于诊断。
