...
坏库、未知 schema、缺文件、无权限、无效时间都会返回非零退出码；不打印空报告掩盖错误。CLI 只读，不会给旧 audit 数据库补列。
...
- **字段缺失**：`fields_missing` 按窗口任务显式统计未知状态、缺完成时间、缺观测快照、缺阶段样本与缺端到端样本；无法从历史 audit/cache 恢复的旧 `created_at` 计入 `created_at`，不使用 API 请求时间猜测。

## 边界与盲区

旧 audit 快照没有 `created_at`、详细笔记状态或 tracker 汇总时，cache 里若仍有同一任务行，可补起点与终态快照字段；任务清理后无法倒推。CLI 不读取共享媒体 `llm_status` 来补任务历史。阶段结果只反映 PerfTracker 已接线的区间，不拆解 LLM 协调器内部子步骤；LLM 用量也不包含未写入 `llm_usage` 的调用。
