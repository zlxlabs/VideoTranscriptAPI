...
### ⚠️ T8 / 端到端真链回归 — 待人工验证（dev 环境无法触达服务）
- **T8 success=false 判定契约**：MediaResolverAPI 服务在本机不可达（`base_url` 为占位
  `host.docker.internal:8000`），无法在线核对 `error.code` 枚举。客户端已**防御式实现**：
  优先按 `error.code`（`_NON_VIDEO_CODES`/`_RESOLVE_FAIL_CODES`）判定，无 code 时按 message 关键词兜底粗分，
  默认归 `ResolverResolveError`。**联调时需对齐真实 code 枚举**，必要时给 MediaResolverAPI 提 issue 加 code。
- **抖音/小红书真链端到端回归**：需真实服务 + 有效 `api_key` + `use_media_resolver=true`，dev 环境缺服务，
  推迟到联调阶段执行（属设计 T9 的真链部分）。单测已用 mock 覆盖全部分支。
