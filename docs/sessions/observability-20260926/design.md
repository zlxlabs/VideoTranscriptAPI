...
3. [实测] CLI 以 audit 快照为主、按 task_id 合并 cache 补充，状态统计每个任务最多计一次；created_at 缺失的历史行仅计入窗口外 `unassigned_created_at_tasks`，不能归属任一窗口。`schema_version` 必须恰有一行整数5或6；v5保留原必需列，v6另需 `created_at` 与 `observability_json`，不迁移数据库。`fields_missing.task_status` 等于 `tasks.status_counts.unknown`，success、failed、queued、processing、calibrating 为已知状态。代码：`scripts/task_observability_report.py`；测试：`tests/unit/test_task_observability_report.py` 覆盖版本/列参数化拒绝、未知状态、去重、v4拒绝、旧 schema、跨窗口未归属行、空窗口及坏库。
