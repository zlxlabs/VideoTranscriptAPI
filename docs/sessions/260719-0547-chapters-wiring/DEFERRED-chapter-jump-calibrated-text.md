...
> 状态：**方向 A 已拍板（2026-07-19），并入 T8 实施**（原计划"先合分支另开任务"调整为不合并、直接在本分支继续；v1 确定性段落化，见 TASKS.md T8 卡与设计 v2.1.2 §6）  
> 相关分支：`feat/chapters-wiring`（接线批）；设计 `docs/plans/2026-07-16-chapter-outline-design.md` v2.1.2
...
| 2026-07-19 | 方向 A 拍板并细化：校对与段落化解耦（先校准后段落化），v1 确定性段落化（长度预算 + 句末/停顿授权），开关默认 false 暗启动；不再另开任务/不先合分支，并入 T8 在本分支实施；详见 TASKS.md T8 卡与设计 v2.1.2 §6 |
| 2026-07-19 | 增量 review 收敛：T8 拍板复用 `dlg-{i}` 锚点，§4「DOM id 统一 seg-{i}」建议作废（改名零收益，且分支未合前即统一） |
